UserEditCourseBook = React.createClass
  ...
  canSubmit: ->
    @state.course_ucode && @state.book_isbn && @state.ok && @state.book_isbn != @state.org_book_isbn
  ...
  handleCourseUcodeChange: (val) ->
    ...
    $.ajax
      ...
      data:
        ...
        include_book: true
    .done (data, textStatus, xhr) =>
      bookISBN = data[0].course_books?[0]?.book_isbn
      if bookISBN
        @setState
          book_isbn: bookISBN
          org_book_isbn: bookISBN
        , =>
          @refs.bookSelect.autoloadAsyncOptions()
  ...
  render: ->
    bookSelectHint = ''
    if @state.book_isbn == @state.org_book_isbn
      bookSelectHint = `<span style={{ color: '#990' }}>請選擇更新後正確的書</span>`
...
window.UserEditCourseBook = UserEditCourseBook
